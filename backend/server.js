          ...
          <td>
            ...
            ">
              ...
              <p style="color:#333; font-size:15px; line-height:1.6; margin-bottom:8px;">
                For security, we strongly recommend updating your password once you sign in to your dashboard and go to the <b>settings</b> section. 
              </p>
...
app.get("/shopify/force-top-level-auth", (req, res) => {
  ...
  // Clear all previous cookies just in case
  const cookieNames = [
    "connect.sid",
    "shopify_app_state",
    "shopify_app_state.sig",
  ...
  // Redirect the browser to /shopify/install after cookie is set
  const redirectUrl = `/shopify/install?shop=${encodeURIComponent(shop)}`;
  res.send(`
    <html><body>
      <script>
        // Wait a tiny moment to ensure the browser registers the cookie
        setTimeout(() => {
...
app.post("/ask-ai", async (req, res) => {
  ...
  try {
    ...
    let aiResponse;
    const { apiKey, message, model = "gpt-5-mini", temperature = 1, ...updates } = req.body;
      ...
      if (!apiKey || !message) {
        aiResponse = "Missing required parameters (userId or message).";
    } else if (!user) {
        aiResponse = "Invalid API key";
    } else if (accountType[0].apiBot === 0) {
        ...
        // booking & conversation state updates here
        // always set aiResponse inside each branch
    }
