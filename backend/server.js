...
app.get("/shopify/callback", async (req, res) => {
  try {
    ...
    // 3️⃣ Run DB + login AFTER sending response (doesn't block Shopify check)
    (async () => {
      try {
        ...
        // ✅ Log user in (after response already sent)
        req.logIn(user, (err) => {
        ...
        console.log("✅ Shopify user created/logged in:", user.email);
      } catch (err) {
