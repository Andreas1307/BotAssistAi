...
useEffect(() => {
    ...
    const fetchSatisfaction = async () => {
      try {
        ...
        if (satisfactionData.length > 0) {
          const totalScore = satisfactionData.reduce(
    ...
    const fetchResTime = async () => {
      ...
      try {
        ...
        for (let i = 0; i < messages.length - 1; i++) {
          if (
            messages[i].sender_type === "user" &&
            messages[i + 1].sender_type === "bot"
          ) {
  ...
  useEffect(() => {
    const fetchDailyConversations = async () => {
      ...
      try {
        ...
        console.log(
          "An error occured with fetching the daily conversations",
          e
        );
        showErrorNotification();
      }
  ...
  useEffect(() => {
    const fetchAllFaq = async () => {
      ...
      try {
        ...
        if (!data.response_tone?.trim())
          issues.push("Response tone is not selected");
        if (!data.business_context?.trim())
          issues.push("Business context is missing");
        if (!data.phoneNum?.trim())
          issues.push("Support phone number is missing");
        if (!data.fine_tuning_data?.trim())
          issues.push("Fine-tuning data is missing");

        // You can also check if uploaded_file is a valid file path or present
        if (!data.uploaded_file || data.uploaded_file.includes("undefined"))
          issues.push("No valid file uploaded");
  ...
  const LogoutConfirmToast2 = ({
    closeToast,
    onConfirm,
    reason = "Are you sure you want to log out?",
  }) => (
    <div>
  ...
  const fetchConvHistory = async (loadAllChats = false) => {
    ...
    try {
      const res = await axios.get(`/conv-history`, {
        params: {
          userId: user.user_id,
          page: loadAllChats ? undefined : page,
          limit: 20,
          all: loadAllChats,
        },
      });
        ...
        if (!res.data.message || res.data.message.length === 0) {
          ...
          setConvHistory((prev) => {
            const newMessages = res.data.message.filter(
              (msg) => !prev.some((prevMsg) => prevMsg.id === msg.id) // Avoid duplicates
            );
